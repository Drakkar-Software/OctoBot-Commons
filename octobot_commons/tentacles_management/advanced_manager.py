        ...
        for child in class_type.__subclasses__():
            if is_abstract(child):
                ...
                if abstract_class is None:
                    _get_advanced(config, child, child.get_name())
                else:
                    _get_advanced(config, child, abstract_class)
    else:
        if abstract_class is not None:
            _append_to_class(config, abstract_class, class_type)
        else:
            _append_to_class(config, class_type.get_name(), class_type)
...
def create_classes_list(config, abstract_class) -> None:
    ...
    :return: None
    """
    _init_class_list_config(config)
    _get_advanced(config, abstract_class)


def _init_class_list_config(config) -> None:
    """
...
def _get_advanced_classes(config) -> dict:
    """
    Get the advanced class from config
    :param config: the config
    :return: the advanced class list
    """
    return config[CONFIG_ADVANCED_CLASSES]


def _get_advanced_instances(config) -> dict:
    """
    Get the advanced instances from config
    :param config: the config
    :return: the advanced instances list
    """
    return config[CONFIG_ADVANCED_INSTANCES]


def _append_to_class(config, class_name, class_type) -> None:
    """
    ...
    """
    try:
        _get_advanced_classes(config)[class_name].append(class_type)
    except KeyError:
        _get_advanced_classes(config)[class_name] = [class_type]


def get_classes(config, class_type, should_get_all_classes=False) -> list:
    """
    Get classes from advanced classes
    :param config: the config
    :param class_type: the class type
    :param should_get_all_classes: if all classes should be taken
    :return: the class list
    """
    classes = []
    if class_type.get_name() in _get_advanced_classes(config):
        classes = copy(_get_advanced_classes(config)[class_type.get_name()])
...
def get_class(config, class_type) -> object:
    """
    Get class from a specified type
    :param config: the config
    :param class_type: the class type
    :return: the class
    """
    ...
    if classes and len(classes) > 1:
        get_logger(__name__).warning(
            f"More than one instance of {class_type} available, " f"using {classes[0]}."
        )
    return classes[0]
...
def get_instance(config, class_type, *args) -> object:
    """
    Get or create the class instance from config
    :param config: the config
    :param class_type: the class type
    :param args: the class args
    :return: the class instance
    """
    advanced_class_type = get_class(config, class_type)
    if class_type in _get_advanced_instances(config):
        return _get_advanced_instances(config)[class_type]
    if advanced_class_type:
        instance = advanced_class_type(*args)
        _get_advanced_instances(config)[class_type] = instance
        return instance
    return None
...
def create_default_types_list(clazz) -> list:
    """
    Create a default types list
    :param clazz: the class
    :return: the default types list
    """
    default_class_list = []
...
def create_advanced_types_list(clazz, config) -> list:
    """
    Create advanced types list
    :param clazz: the class
    :param config: the config
    :return: the advanced types list
    """
    advanced_class_list = [
        class_type
    ...
    if not _check_duplicate(advanced_class_list):
...
def get_all_classes(clazz, config) -> list:
    ...
    :param clazz: the class
    :param config: the config
    :return: the classes from config list
    """
    return [
        class_type
        for subclass in clazz.__subclasses__()
...
def _check_duplicate(list_to_check) -> bool:
    """
    Check if a list has duplicate
    :param list_to_check: the list to check
    :return: True if the list has duplicate values
    """
