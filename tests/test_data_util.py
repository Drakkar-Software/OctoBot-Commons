...
from octobot_commons.data_util import drop_nan, mean, shift_value_array, HL2, HLC3, OHLC4, HeikinAshi
...
def test_HL2():
    high = np.array([10, 12, np.nan, 45, 5.67, 6.54, 75, 8.01, 9])
    low = np.array([9, 8, 7, 6, 5, 4, 3, 2, 1])
    np.testing.assert_array_equal(HL2(high, low),
                                  np.array([9.5, 10, np.nan, 25.5, 5.335, 5.27, 39.0, 5.005, 5.0], dtype=np.float64))
    
    high = np.array([120, 123, 54, 45, 210.54, 546.21, 981.2, .958, 65.7])
    low = np.array([887.592, 896.519, 97.416, 233.987, 846.789, 713.054, 856.985, 421.17, 874.296])
    np.testing.assert_array_equal(HL2(high, low),
...
def test_HLC3():
    high = np.array([9, 13, np.nan, 45, 5.67, 6.54, 75, 8.01, 9])
    low = np.array([19, 25, 17, 36, 45, 84, 31, 21, 10])
    close = np.array([2, 4, 4, 4, 6, 7, 8, 9, 10])
    np.testing.assert_array_equal(HLC3(high, low, close),
                                  np.array([10, 14, np.nan, 28.333333333333332, 18.89, 
                                  32.513333333333335, 38, 12.67, 9.666666666666666], dtype=np.float64))

    high = np.array([733.985, 86.751, 388.834, 630.849, 231.102, 224.815, 430.74, 776.919, 209.207])
    low = np.array([145.747, 829.698, 534.426, 879.53, 187.895, 698.515, 822.942, 532.641, 626.917])
    close = np.array([811.199, 278.313, 817.295, 315.199, 974.104, 775.321, 979.139, 790.477, 518.736])
    np.testing.assert_array_equal(HLC3(high, low, close),
...
def test_OHLC4():
    open = np.array([251.613, 259.098, 247.819, 140.73, 237.547, 830.611, 433.168, 404.026, 403.538])
    high = np.array([980.99, 403.92, 698.072, 658.647, 245.151, 480.9, 621.35, 429.109, 637.439])
    low = np.array([658.777, 101.13, 549.588, 28.624, 132.07, 813.572, 366.478, 619.649, 371.696])
    close = np.array([812.829, 880.456, 406.039, 39.224, 917.386, 707.281, 737.851, 330.262, 258.689])
    np.testing.assert_array_equal(OHLC4(open, high, low, close),
    ...
    open = np.array([345.468, 484.778, 332.855, 401.893, 41.936, 333.738, 983.158, 996.979, 807.855])
    high = np.array([547.277, 856.206, 439.542, 921.475, 778.994, 156.285, 653.31, 534.865, 427.64])
    low = np.array([328.444, 593.535, 4.243, 83.902, 811.859, 396.442, 433.552, 127.624, 314.613])
    close = np.array([905.792, 382.98, 135.529, 494.942, 510.52, 399.78, 897.088, 192.068, 771.189])
    np.testing.assert_array_equal(OHLC4(open, high, low, close), np.array([531.74525, 579.37475, 228.04225, 
                                    475.553, 535.82725, 321.56125, 741.777, 462.884, 580.32425], dtype=np.float64))

def test_HeikinAshi():
    open = np.array([977.88, 573.634, 816.233, 846.748, 184.114, 35.742, 598.653, 745.916, 854.334])
    high = np.array([4.757, 499.759, 602.794, 179.313, 802.019, 384.307, 637.378, 161.048, 366.51])
    low = np.array([903.152, 877.832, 966.154, 104.582, 837.638, 568.788, 788.584, 510.926, 608.184])
    close = np.array([405.527, 685.962, 495.698, 271.687, 573.667, 891.018, 445.342, 344.928, 894.279])
    ...
    open = np.array([188.539, 334.682, 495.604, 638.736, 632.213, 705.675, 876.735, 69.951, 909.477])
    high = np.array([259.316, 843.705, 170.388, 318.961, 918.236, 585.595, 23.266, 657.422, 270.557])
    low = np.array([652.361, 293.607, 295.191, 893.255, 819.447, 647.016, 330.303, 472.415, 617.705])
    close = np.array([968.007, 114.792, 680.216, 168.147, 478.577, 437.676, 299.474, 208.601, 333.237])
